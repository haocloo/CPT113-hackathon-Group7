...
1) Clone this game using your terminal
```bash
git clone https://github.com/haocloo/CPT113-hackathon-Group7
```
...
---
### 1️⃣ Move between rooms, pick up items, use items, and check your inventory
...
### 2️⃣ Solve riddles, unlock hidden rooms, and unlock the treasure chest with the necessary items

- The user can solve riddles using some items that contain clues or questions. For example, the riddle book contains a riddle that can be answered by typing the correct word. The card1, card2, and card3 contain questions that can be answered by typing the correct answer. Solving riddles may reveal hints or passwords for the treasure chest or hidden rooms.
...
- Room *entrance = new Room("Entrance", "You stand at the entrance of a mysterious mansion...");
...
- The Room class has public methods such as getName(), getDescription(), getItem(), getTreasure(), getHidden(), setName(), setDescription(), setItem(), setTreasure(), setHidden(), and printList() that provide an interface for other classes or functions to interact with the Room object.
